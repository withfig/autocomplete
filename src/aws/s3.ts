    ...
    },
    {
      ...
      options: [
        ...
        },
        {
          name: "--storage-class",
          description:
            "The type of storage to use for the object. Valid choices are: STANDARD | REDUCED_REDUNDANCY | STANDARD_IA | ONEZONE_IA | INTELLIGENT_TIERING | GLACIER | DEEP_ARCHIVE | GLACIER_IR. Defaults to 'STANDARD'",
          args: {
            name: "string",
            suggestions: [
              "STANDARD",
        ...
        },
        {
          name: "--metadata",
          description:
            "A map of metadata to store with the objects in S3. This will be applied to every object which is part of this request. In a sync, this means that files which haven't changed won't receive the new metadata",
          args: {
            name: "map",
          },
        },
        {
          name: "--copy-props",
          description:
            "Determines which properties are copied from the source S3 object. This parameter only applies for S3 to S3 copies. Valid values are: ``none`` - Do not copy any of the properties from the source S3 object.``metadata-directive`` - Copies the following properties from the source S3 object: ``content-type``, ``content-language``, ``content-encoding``, ``content-disposition``, ``cache-control``, ``--expires``, and ``metadata````default`` - The default value. Copies tags and properties covered under the ``metadata-directive`` value from the source S3 object.In order to copy the appropriate properties for multipart copies, some of the options may require additional API calls if a multipart copy is involved. Specifically:``metadata-directive`` may require additional ``HeadObject`` API calls.``default`` may require additional ``HeadObject``, ``GetObjectTagging``, and ``PutObjectTagging`` API calls. Note this list of API calls may grow in the future in order to ensure multipart copies preserve the exact properties a ``CopyObject`` API call would preserve.If you want to guarantee no additional API calls are made other than than the ones needed to perform the actual copy, set this option to ``none``",
          args: {
            name: "string",
            suggestions: ["none", "metadata-directive", "default"],
          },
        },
        {
