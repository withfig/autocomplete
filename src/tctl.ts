...
const flatArrayToTeleportRoles = (roles: string[]): { "metadata": { "name": string } }[] => {
...
const teleportRolesToFlatArray = (roles: { "metadata": { "name": string } }[]): string[] => {
  return roles.map((role) => {
    return role.metadata.name;
  })
}

const filterRoles = (currentRoles: string, allRoles: { "metadata": { "name": string, "description": string } }[]) => {
...
const teleportWindowsDesktopGenerator: Fig.Generator = {
  ...
  postProcess: function (out) {
    ...
    return desktops.map((desktop: any) => {
      return {
...
const teleportGetResourcesGenerator: Fig.Generator = {
  ...
  custom: async (tokens, executeShellCommand) => {
    const standardSuggestions = ["user", "role", "connector", "node", "windows_desktop", "cluster", "login_rule", "device", "ui_config", "cluster_auth_preference", "lock", "all"]
    let respondSuggestions = standardSuggestions.map((suggestion) => {
      return {
        name: suggestion,
    ...
    if (tokens.find((token) => standardSuggestions.includes(token.split("/")[0])) != undefined) {
      // Get the name from the token which contains one of the standard suggestions
      const resource = tokens.find((token) => standardSuggestions.includes(token.split("/")[0])).split("/")[0];

      if (standardSuggestions.find((sug) => sug === resource) == undefined) return respondSuggestions;
      if (["cluster_auth_preference", "all"].includes(resource)) return []; // This is what tctl expects

      const resources = await executeShellCommand(`tctl get ${resource} --format json`);
      const parsedResources = JSON.parse(resources);

      return parsedResources.map((parsedResource: any) => {
        return {
...
const completionSpec: Fig.Spec = {
  ...
  subcommands: [
      ...
      subcommands: [
        {
          ...
          options: [
            ...
            {
              name: "--db-roles",
              description: "List of database roles for automatic database user provisioning."
            },
            {
              name: "--aws-role-arns",
            ...
            },
            {
              ...
              description: "List of allowed GCP service accounts for the new user.",
            },
            {
              name: "--host-user-uid",
            ...
            {
              name: "--ttl",
              description: "Set expiration time for token, default is 1h0m0s, maximum is 48h0m0s.",
            },
            {
              name: "--roles",
              description: "List of roles for the new user to assume. Comma seperated.",
              isRequired: true,
              isRepeatable: true,
              args: {
        ...
        },
        {
          ...
          options: [
            {
              name: "--set-roles",
              description: "List of roles for the user to assume, replaces current roles. Comma seperated.",
              args: {
                generators: teleportRolesGenerator
              }
            },
            {
              name: "--set-logins",
              description: "List of allowed SSH logins for the user, replaces current logins.",
            },
            {
              name: "--set-windows-logins",
              description: "List of allowed Windows logins for the user, replaces current Windows logins.",
            },
            {
              name: "--set-kubernetes-users",
              description: "List of allowed Kubernetes users for the user, replaces current Kubernetes users.",
            },
            {
              name: "--set-kubernetes-groups",
              description: "List of allowed Kubernetes groups for the user, replaces current Kubernetes groups.",
            },
            {
              name: "--set-db-users",
              description: "List of allowed database users for the user, replaces current database users.",
            },
            {
              name: "--set-db-names",
              description: "List of allowed database names for the user, replaces current database names.",
            },
            {
              name: "--set-db-roles",
              description: "List of allowed database roles for automatic database user provisioning, replaces current database roles.",
            },
            {
              name: "--set-aws-role-arns",
              description: "List of allowed AWS role ARNs for the user, replaces current AWS role ARNs.",
            },
            {
              name: "--set-azure-identities",
              description: "List of allowed Azure identities for the user, replaces current Azure identities.",
            },
            {
              name: "--set-gcp-service-accounts",
              description: "List of allowed GCP service accounts for the user, replaces current service accounts.",
            },
            {
              name: "--set-host-user-uid",
              description: "UID for auto provisioned host users to use. Value can be reset by providing an empty string.",
            },
            {
              name: "--set-host-user-gid",
              description: "GID for auto provisioned host users to use. Value can be reset by providing an empty string.",
            }
          ],
          args: {
            name: "account",
        ...
        {
          name: "reset",
          description: "Reset user password and generate a new token [Teleport DB users only].",
          args: {
            name: "account",
            description: "Teleport user account name.",
            generators: teleportAccountsGenerator
      ...
      subcommands: [
        {
          ...
          options: [
            {
              name: "--roles",
              description: "Comma-separated list of roles for the new node to assume",
              args: {
                generators: {
                  getQueryTerm: commaQueryTerm,
                  trigger: (current, old) => {
                  ...
                  custom: async (tokens) => {
                    ...
                    const filteredRoles = filterRoles(tokens[tokens.length - 1], roles.map((role) => {
                      return {
                        "metadata": {
                          "name": role,
                          "description": "Add a new " + role + " to the cluster"
                        }
                      }
                    }));
            ...
            {
              name: "--ttl",
              description: "Time to live for a generated token, default is 0h30m0s, maximum is 48h0m0s.",
            }
          ]
        },
        {
      ...
      subcommands: [
        {
          ...
          options: [
                  ...
                  custom: async (tokens) => {
                    const types = [
                      ...
                      },
                      {
                        "metadata": {
                          "name": "auth",
                          "description": "Add a new auth service node to the cluster"
                        }
                      },
                      {
                        "metadata": {
                          "name": "trusted_cluster",
                          "description": "Add a new trusted cluster to the cluster"
                        }
                      },
                      {
                        "metadata": {
                      ...
                      },
                      {
                        "metadata": {
                          "name": "kube",
                          "description": "Add a new kubernetes cluster to the cluster"
                        }
                      },
                      {
                        "metadata": {
                          "name": "app",
                          "description": "Add a new (web-)application to the cluster"
                        }
                      },
                      {
                        "metadata": {
                          "name": "windowsdesktop",
                          "description": "Add a new windows desktop to the cluster"
                        }
                      }
                    ]

                    const filteredTypes = filterRoles(tokens[tokens.length - 1], types);
            ...
            {
              name: "--value",
              description: "Override the default random generated token with a specified value",
              args: {
                name: "value",
              }
            },
            ...
            {
              name: "--ttl",
              description: "Set expiration time for token, default is 30 minutes",
              args: {
                name: "30m",
              }
            },
            ...
            {
              name: "--db-protocol",
              description: "Database protocol to use. Supported are: [postgres mysql mongodb oracle cockroachdb redis snowflake sqlserver cassandra elasticsearch opensearch dynamodb clickhouse clickhouse-http]",
              args: {
                name: "protocol",
                suggestions: ["postgres", "mysql", "mongodb", "oracle", "cockroachdb", "redis", "snowflake", "sqlserver", "cassandra", "elasticsearch", "opensearch", "dynamodb", "clickhouse", "clickhouse-http"],
                default: "postgres",
              }
            }
          ]
      ...
      subcommands: [
        {
          ...
          options: [
            ...
            {
              name: "--compat",
              description: "Export certificates compatible with specific version of Teleport",
              args: {
                name: "version",
                description: "E.g. 13"
              }
            },
            {
              ...
              args: {
                name: "type",
                suggestions: ["user", "host", "tls-host", "tls-user", "tls-user-der", "windows", "db", "db-der", "openssh", "saml-idp"],
                description: "Export certificate type",
              }
            }
          ]
        },
        {
          ...
          options: [
            ...
            {
              name: "--proxy",
              description: "Address of the Teleport proxy. When --format is set to \"kubernetes\", this address will be set as cluster address in the generated kubeconfig file",
              args: {
                name: "proxy",
              }
            },
            {
              name: "--overwrite",
              description: "Whether to overwrite existing destination files. When not set, user will be prompted before overwriting any existing file.",
            },
            {
              name: "--no-overwrite",
              description: "Whether to overwrite existing destination files. When not set, user will be prompted before overwriting any existing file.",
            ...
            {
              name: "--tar",
              description: "Create a tarball of the resulting certificates and stream to stdout.",
            },
            {
              name: "--leaf-cluster",
              description: "Leaf cluster to generate identity file for when --format is set to \"kubernetes\"",
              args: {
                name: "leaf-cluster",
              }
            },
            {
              ...
              description: "Kubernetes cluster to generate identity file for when --format is set to \"kubernetes\"",
              args: {
                name: "name",
            ...
            {
              name: "--app-name",
              description: "Application to generate identity file for. Mutually exclusive with \"--db-service\".",
              args: {
                name: "name",
            ...
            {
              name: "--db-service",
              description: "Database to generate identity file for. Mutually exclusive with \"--app-name\".",
              args: {
                name: "service",
            ...
            {
              name: "--db-user",
              description: "Database user placed on the identity file. Only used when \"--db-service\" is set.",
            },
            {
              name: "--db-name",
              description: "Database name placed on the identity file. Only used when \"--db-service\" is set.",
            },
            {
              name: "--windows-user",
              description: "Window user placed on the identity file. Only used when --format is set to \"windows\"",
            },
            {
              name: "--windows-domain",
              description: "Active Directory domain for which this cert is valid. Only used when --format is set to \"windows\"",
            },
            {
              name: "--windows-sid",
              description: "Optional Security Identifier to embed in the certificate. Only used when --format is set to \"windows\"",
            }
          ]
        },
        {
          ...
          options: [
            {
              name: "--grace-period",
              description: "Grace period keeps previous certificate authorities signatures valid, if set to 0 will force users to re-login and nodes to re-register.",
              args: {
                name: "duration",
            ...
            {
              name: "--manual",
              description: "Activate manual rotation, set rotation phases manually",
            },
            {
              name: "--type",
              ...
              args: {
                name: "type",
                suggestions: ["host", "user", "db", "openssh", "jwt", "saml_idp", "oidc_idp"],
                default: "host",
              }
            },
            {
              name: "--phase",
              description: "Target rotation phase to set, used in manual rotation",
              args: {
                name: "phase",
                suggestions: ["init", "standby", "update_clients", "update_servers", "rollback"],
                default: "init",
              }
            }
          ]
        ...
        {
          name: "crl",
          description: "Export empty certificate revocation list (CRL) for certificate authorities.",
          options: [
            {
              name: "--type",
              description: "Certificate authority to rotate",
    ...
    /* tctl get */
    {
      ...
      options: [
        ...
        {
          name: "--with-secrets",
          description: "Include secrets in resources like certificate authorities or OIDC connectors",
        },
        {
          name: "--verbose",
    ...
    /* tctl kube */
    {
      ...
      subcommands: [
        {
          name: "ls",
          description: "List all Kubernetes clusters registered with the cluster.",
        }
      ]
    },
    ...
    {
      name: "bots",
      description: "Operate on certificate renewal bots registered with the cluster.",
      requiresSubcommand: true,
      subcommands: [
        {
          name: "ls",
          description: "List all certificate renewal bots registered with the cluster.",
        },
        {
          name: "add",
        ...
        {
          name: "rm",
          description: "Permanently remove a certificate renewal bot from the cluster.",
          args: {
            name: "name",
          ...
          options: [
            ...
            },
            {
              ...
              args: {
                ...
                suggestions: ["node", "kube", "proxy", "auth", "app", "db", "windowsdesktop"]
              }
            },
            {
              name: "--upgrader",
      ...
      subcommands: [
        {
          ...
          options: [
            ...
            {
              name: ["--verbose", "-v"],
              description: "Show detailed alert info, including acknowledged alerts."
            }
          ]
        },
        {
          ...
          options: [
            ...
            {
              name: "--ttl",
              description: "Time duration after which the alert expires (default 24h).",
            }
          ],
          args: {
            name: "message",
      ...
      subcommands: [
        {
          ...
          options: [
            ...
            {
              name: "--asset-tag",
              description: "Inventory identifier for the device (e.g., Mac serial number)",
              exclusiveOn: ["--current-device"]
            },
            {
              name: "--current-device",
              description: "Registers the current device. Overrides --os and --asset-tag.",
          ...
          subcommands: [
            {
              ...
              args: [
                ...
                {
                  name: "expires",
                  description: "When the user's access expires (must be in RFC3339). Defaults to the expiration time of the access list.",
                  isOptional: true,
                },
                {
                  name: "reason",
                  description: "The reason the user has been added to the access list. Defaults to empty.",
                  isOptional: true,
                }
              ]
            },
    ...
    {
      name: "sso",
      description: "A family of commands for configuring and testing auth connectors (SSO).",
      requiresSubcommand: true,
      subcommands: [
        {
          name: "configure",
          ...
          subcommands: [
            {
              ...
              options: [
                ...
                {
                  name: "--teams-to-roles",
                  description: "Sets teams-to-roles mapping using format 'organization,name,role1,role2,...'. Repeatable.",
                  args: {
                    name: "teams-to-roles",
                  }
                },
                ...
                },
                {
                  ...
                  description: "Ignore missing roles referenced in --teams-to-roles.",
        ...
        {
          name: "test",
          description: "Perform end-to-end test of SSO flow using provided auth connector definition.",
          args: {
            name: "filename",
            description: "Connector resource definition filename. Empty for stdin.",
            isOptional: true,
            template: "filepaths",
          }
        },
  ...
  options: [
    ...
    {
      name: ["--insecure"],
      description: "When specifying a proxy address in --auth-server, do not verify its TLS certificate.",
      isPersistent: true,
    }
  ],
