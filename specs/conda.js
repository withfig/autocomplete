    ...
    subcommands: [
        ...
        },
        {
            ...
            options: [
                ...
                { name: ["--system"], description: "Write to the system .condarc file at '/home/docs/checkouts/readthedocs.org/user_builds/continuumio-conda/conda/latest/.condarc'." },
                { name: ["--env"], description: "Write to the active conda environment .condarc file (<no active environment>). If no environment is active, write to the user config file (/home/docs/.condarc)." },
                { name: ["--file"], description: "Write to the given file." },
                { name: ["--show"], description: "Display configuration values as calculated and compiled. If no arguments given, show information for all configuration values." },
                ...
                { name: ["--describe"], description: "Describe given configuration parameters. If no arguments given, show information for all configuration parameters." },
                { name: ["--write-default"], description: "Write the default configuration to a file. Equivalent to conda config --describe > ~/.condarc." },
                {
                    name: ["--get"],
        ...
        },
        {
            ...
            options: [
                ...
                { name: ["--dev"], description: "Use sys.executable -m conda in wrapper scripts instead of CONDA_EXE. This is mainly for use during tests where we test new conda source against old Python versions." },
                { name: ["-n, --name"], description: "Name of environment." },
                { name: ["-p, --prefix"], description: "Full path to environment location (i.e. prefix)." },
                { name: ["-c, --channel"], description: "they are given (including local directories using the 'file://' syntax or simply a path like '/home/conda/mychan' or '../mychan'). Then, the defaults or channels from .condarc are searched (unless --override-channels is given). You can use 'defaults' to get the default packages for conda. You can also use any name and the .condarc channel_alias value will be prepended. The default channel_alias is http://conda.anaconda.org/." },
                ...
                { name: ["--repodata-fn"], description: "Specify name of repodata on remote server. Conda will try whatever you specify, but will ultimately fall back to repodata.json if your specs are not satisfiable with what you specify here. This is used to employ repodata that is reduced in time scope. You may pass this flag more than once. Leftmost entries are tried first, and the fallback to repodata.json is added for you automatically." },
                { name: ["--strict-channel-priority"], description: "Packages in lower priority channels are not considered if a package with the same name appears in a higher priority channel." },
                { name: ["--no-channel-priority"], description: "Package version takes precedence over channel priority. Overrides the value given by conda config --show channel_priority." },
                { name: ["--no-deps"], description: "Do not install, update, remove, or change dependencies. This WILL lead to broken environments and inconsistent behavior. Use at your own risk." },
                ...
                { name: ["-C, --use-index-cache"], description: "Use cache of channel index files, even if it has expired." },
                { name: ["-k, --insecure"], description: "Allow conda to perform insecure SSL connections and transfers. Equivalent to setting 'ssl_verify' to 'false'." },
                ...
                { name: ["--json"], description: "Report all output as json. Suitable for using conda programmatically." },
                { name: ["-q, --quiet"], description: "Do not display progress bar." },
                { name: ["-v, --verbose"], description: "Can be used multiple times. Once for INFO, twice for DEBUG, three times for TRACE." },
                { name: ["-y, --yes"], description: "Do not ask for confirmation." },
                { name: ["--download-only"], description: "Solve an environment and ensure package caches are populated, but exit prior to unlinking and linking packages into the prefix." },
                { name: ["--show-channel-urls"], description: "Show channel urls. Overrides the value given by conda config --show show_channel_urls." },
            ]
        },
        {
            name: "help",
        ...
        },
        {
            ...
            options: [
                ...
                { name: ["--only-deps"], description: "Only install dependencies." },
                { name: ["--no-pin"], description: "Ignore pinned file." },
                { name: ["--force-reinstall"], description: "Ensure that any user-requested package for the current operation is uninstalled and reinstalled, even if that package already exists in the environment." },
                { name: ["--freeze-installed", "--no-update-deps"], description: "Do not update or change already-installed dependencies." },
                { name: ["--update-deps"], description: "Update dependencies." },
                { name: ["-S", "--satisfied-skip-solve"], description: "Exit early and do not run the solver if the requested specs are satisfied. Also skips aggressive updates as configured by 'aggressive_update_packages'. Similar to the default behavior of 'pip install'." },
                ...
                { name: ["--clobber"], description: "Allow clobbering of overlapping file paths within packages, and suppress related warnings." },
                { name: ["-C", "--use-index-cache"], description: "Use cache of channel index files, even if it has expired." },
                { name: ["-k", "--insecure"], description: "Allow conda to perform insecure SSL connections and transfers. Equivalent to setting 'ssl_verify' to 'false'." },
                { name: ["--offline"], description: "Offline mode. Don't connect to the Internet." },
                { name: ["-d", "--dry-run"], description: "Only display what would have been done." },
        ...
        },
        {
            ...
            options: [
                { name: ["--show-channel-urls"], description: "Show channel urls. Overrides the value given by conda config --show show_channel_urls." },
                { name: ["-c", "--canonical"], description: "Output canonical names of packages only. Implies --no-pip." },
                { name: ["-f", "--full-name"], description: "Only search for full names, i.e., ^<regex>$." },
                { name: ["--explicit"], description: "List explicitly all installed conda packaged with URL (output may be used by conda create --file)." },
                { name: ["-e", "--export"], description: "Output requirement string only (output may be used by conda create --file)." },
        ...
        },
        {
            ...
            options: [
                { name: ["-w", "--which"], description: "Given some PATH print which conda package the file came from." },
        ...
        },
        {
            ...
            options: [
                ...
                { name: ["--force-remove, --force"], description: "Forces removal of a package without removing packages that depend on it. Using this option will usually leave your environment in a broken and inconsistent state." },
                { name: ["--no-pin"], description: "Ignore pinned file." },
                { name: ["-C", "--use-index-cache"], description: "Use cache of channel index files, even if it has expired." },
                { name: ["-k", "--insecure"], description: "Allow conda to perform insecure SSL connections and transfers. Equivalent to setting 'ssl_verify' to 'false'." },
                { name: ["--offline"], description: "Offline mode. Don't connect to the Internet." },
        ...
        },
        {
            ...
            args: { name: "query" },
            options: [
                { name: ["--envs"], description: "Search all of the current user's environments. If run as Administrator (on Windows) or UID 0 (on unix), search all known environments on the system." },
                { name: ["-i", "--info"], description: "Provide detailed information about each package." },
                { name: ["--subdir", "--platform"], description: "Search the given subdir. Should be formatted like 'osx-64', 'linux-32', 'win-64', and so on. The default is to search the current platform." },
                { name: ["-c", "--channel"], description: "they are given (including local directories using the 'file://' syntax or simply a path like '/home/conda/mychan' or '../mychan'). Then, the defaults or channels from .condarc are searched (unless --override-channels is given). You can use 'defaults' to get the default packages for conda. You can also use any name and the .condarc channel_alias value will be prepended. The default channel_alias is http://conda.anaconda.org/." },
                { name: ["--use-local"], description: "Use locally built packages. Identical to '-c local'." },
                { name: ["--override-channels"], description: "Do not search default or .condarc channels. Requires --channel." },
                { name: ["--repodata-fn"], description: "Specify name of repodata on remote server. Conda will try whatever you specify, but will ultimately fall back to repodata.json if your specs are not satisfiable with what you specify here. This is used to employ repodata that is reduced in time scope. You may pass this flag more than once. Leftmost entries are tried first, and the fallback to repodata.json is added for you automatically." },
