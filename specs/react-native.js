...
var workerGenerator = {
  ...
  postProcess: function (scriptOutput) {
    return Array.from({ length: Number(scriptOutput) }, function (_x, i) {
      return {
        name: "" + i,
      };
    });
...
var xcodeConfigGenerator = {
  ...
  postProcess: function (scriptOutput) {
    ...
    return configurations.map(function (name) {
      return { name: name };
    });
  },
};
  ...
  postProcess: function (scriptOutput) {
    var devices = scriptOutput
      ...
      .filter(function (item) {
        return !item.match(/^(List)|\*/);
      })
      .filter(Boolean)
      .filter(function (item) {
        return item.match(/device$/);
      })
      .map(function (item) {
        return item.split(/device/)[0].trim();
      });
