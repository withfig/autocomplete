var completionSpec = {
    ...
    options: [
      ...
      {
        name: [ '-S' ],
        description: "Makes Ruby use the PATH environment variable to search for script, unless if its name begins with a slash. This is used to emulate #! on machines that don't support it, in the following manner: #! /usr/local/bin/ruby # This line makes the next one a comment in Ruby \\ exec /usr/local/bin/ruby -S $0 $*"
      },
      {
        name: [ '-T' ],
      ...
      { name: [ '-s' ], description: 'Enables some switch parsing for switches after script name but before any file name arguments (or before a --).' },
      {
        name: [ '-v', '--verbose' ],
